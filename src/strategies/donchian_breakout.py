...
class DonchianBreakoutStrategy(BaseStrategy):
    """
    Auto-translated from PineScript: Donchian Breakout
    ...
    def generate_signals(self, df):
        ...
        try:
            upperBand = df['high'].rolling(self.params.get('lookback', 20)).max()
        except Exception:
            upperBand = pd.Series(np.nan, index=df.index, dtype='float64')
        try:
            lowerBand = df['low'].rolling(self.params.get('lookback', 20)).min()
        except Exception:
            lowerBand = pd.Series(np.nan, index=df.index, dtype='float64')
        try:
            longCond = df['close'] > upperBand
        except Exception:
            longCond = pd.Series(np.nan, index=df.index, dtype='float64')
        try:
            shortCond = df['close'] < lowerBand
        except Exception:
