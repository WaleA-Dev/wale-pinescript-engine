...
class MacdCrossStrategy(BaseStrategy):
    """
    Auto-translated from PineScript: MACD Cross
    ...
    def __init__(self, **params):
        ...
        self.params.setdefault('fastPeriod', 12)
        self.params.setdefault('slowPeriod', 26)
        self.params.setdefault('signalPeriod', 9)
    ...
    def generate_signals(self, df):
        ...
        try:
            macdVal = self._calc_macd(df['close'])[0]
        except Exception:
            macdVal = pd.Series(np.nan, index=df.index, dtype='float64')
        try:
            longCond = ((macdVal) > (0)) & ((macdVal).shift(1) <= 0)
        except Exception:
            longCond = pd.Series(np.nan, index=df.index, dtype='float64')
        try:
            shortCond = ((macdVal) < (0)) & ((macdVal).shift(1) >= 0)
        except Exception:
